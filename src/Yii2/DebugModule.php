...
use Magput\Debug\Yii2\Data\DataStorageInterface;
use Magput\Debug\Yii2\Data\FileDataStorage;
use Yii;
 ...
 * @since  2.0
 *
 * @property-read string $toolbarHtml
 * @property-write Event $debugHeaders
 */
class DebugModule extends Module implements BootstrapInterface
{
     ...
     * {@inheritdoc}
     */
    public string $controllerNamespace = 'Magput\Debug\Yii2\Controllers';
    /**
     * @var DebugTarget|array|string the logTarget object, or the configuration for creating the logTarget object.
     */
    public $logTarget = DebugTarget::class;
    /**
     ...
     * @var string[]
     */
    public array $dataStorageConfig = [
    ...
    public function init()
    {
        ...
        $this->dataStorage = Instance::ensure($this->dataStorageConfig + ['module' => $this],DataStorage::class);
    ...
    public function bootstrap($app)
    {
        if (is_array($this->logTarget)) {
            if (!isset($this->logTarget['class'])) {
                $this->logTarget['class'] = DebugTarget::class;
            }
    ...
    {
        $packageInfo = Json::decode(file_get_contents(dirname(__DIR__) . DIRECTORY_SEPARATOR));
