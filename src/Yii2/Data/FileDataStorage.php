...
class FileDataStorage extends Component implements DataStorageInterface
{
    /**
     * @var string
     */
    ...
    public function getDataManifest($forceReload = false)
    {
        if ($this->_manifest === null || $forceReload) {
            ...
            if ($content !== '') {
                $this->_manifest = array_reverse(json_decode($content, true), true);
            } else {
                $this->_manifest = [];
            }
        }
    ...
    private function updateIndexFile($indexFile, $tag, $summary)
    {
        ...
        ftruncate($fp, 0);
        rewind($fp);
        fwrite($fp, json_encode($manifest, JSON_UNESCAPED_UNICODE));
